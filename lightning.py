...
class ModelModule(LightningModule):
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW([{"name": "model", "params": self.model.parameters(
        ), "lr": self.cfg.optimizer.lr}], weight_decay=self.cfg.optimizer.weight_decay, betas=(0.9, 0.98))
        scheduler = WarmupCosineScheduler(optimizer, self.cfg.optimizer.warmup_epochs, self.cfg.trainer.max_epochs, len(
    ...
    def forward(self, sample):
        ...
        #         list(map(int, hyp.asdict()["yseq"][1:])))
        #     predicted = self.text_transform.post_process(
        #         predicted_token_id).replace("<eos>", "")
        #     print(f"Prediction {i}: {predicted} | scores: {hyp_dict['scores']}")
    ...
    def test_step(self, sample, sample_idx):
        ...
        nbest_hyps = self.beam_search(enc_feat)
        nbest_hyps = [h.asdict()
                               for h in nbest_hyps[: min(len(nbest_hyps), 1)]]
        predicted_token_id = torch.tensor(
            list(map(int, nbest_hyps[0]["yseq"][1:])))
        predicted = self.text_transform.post_process(
            predicted_token_id).replace("<eos>", "")
    ...
    def _step(self, batch, batch_idx, step_type):
        ...
        if step_type == "train":
            ...
            self.log("loss_ctc", loss_ctc, on_step=False,
                     on_epoch=True, batch_size=batch_size)
            self.log("loss_att", loss_att, on_step=False,
                     on_epoch=True, batch_size=batch_size)
            self.log("decoder_acc", acc, on_step=True,
                     on_epoch=True, batch_size=batch_size)
        else:
        ...
        if step_type == "train":
            self.log("monitoring_step", torch.tensor(
                self.global_step, dtype=torch.float32))
