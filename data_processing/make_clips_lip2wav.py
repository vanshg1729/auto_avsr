...
def process_video_file(video_path, args, job_id=0, video_id=0):
    ...
    for track_id, track in enumerate(tracks_list):
        ...
        track_clips = []
        aligned_track_clips = align_track_to_segments(track, aligned_segments, min_clip_len=0.5, word_level=True)
...
def main(args):
    ...
    print(f"Number of files for job {args.job_index} index: {len(video_files)}")
